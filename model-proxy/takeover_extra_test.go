...
func TestColorHelpers_NoColorPassthrough(t *testing.T) {
	...
	// (and NO_COLOR may be set), so color helpers return the input unchanged.
	// Assert the no-color branch is exercised and returns the string verbatim.
	for _, s := range []string{"x", "hello"} {
		if got := cYellow(s); got != s && !strings.Contains(got, s) {
			t.Errorf("cYellow(%q)=%q lost input", s, got)
		}
		if got := cMagenta(s); got != s && !strings.Contains(got, s) {
			t.Errorf("cMagenta(%q)=%q lost input", s, got)
		}
	}
}
