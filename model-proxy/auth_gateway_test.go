...
// AuthProvider implementations) + gateway.go (aqp client helpers) +
...
func TestNewAuthProvider_Factory(t *testing.T) {
	cfg := &Config{
		Providers: map[string]Provider{
			"aqp":        {Provider: "aqp", AqpMintURL: "https://x/mint"},
...
// --- AqpKeyProvider: key minting via mock + caching + refresh ---

func TestAqpKeyProvider_MintAndCache(t *testing.T) {
	mints := 0
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		mints++
		w.Write([]byte(`{"retcode":0,"data":{"api_key":"aqp-key-12345","project_id":"pid","employee_email":"u@x"}}`))
	}))
	defer srv.Close()

	dir := t.TempDir()
	store := filepath.Join(dir, "aqp_oauth_auth.json")
	...
	if got := req.Header.Get("Authorization"); got != "Bearer aqp-key-12345" {
		t.Errorf("AQP Inject=%q want Bearer aqp-key-12345", got)
	}
	// Second Inject uses cache (no second mint).
	req2, _ := http.NewRequest("GET", "https://x", nil)
	p.Inject(req2)
	if mints != 1 {
		t.Errorf("AQP mints=%d want 1 (cached)", mints)
	}
	...
	p.Inject(req3)
	if mints != 2 {
		t.Errorf("AQP mints=%d want 2 (after refresh)", mints)
	}
}
...
func TestAqpKeyProvider_Non200(t *testing.T) {
	...
	req, _ := http.NewRequest("GET", "https://x", nil)
	if err := p.Inject(req); err == nil {
		t.Error("AQP Inject on 500: want error, got nil")
	}
}

func TestAqpKeyProvider_BadRetcode(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	if err := p.Inject(req); err == nil || !strings.Contains(err.Error(), "retcode") {
		t.Errorf("AQP Inject bad retcode: err=%v want retcode error", err)
	}
}

func TestAqpKeyProvider_NoSSOCookie(t *testing.T) {
	dir := t.TempDir()
	store := filepath.Join(dir, "aqp_oauth_auth.json") // absent
	p := newAqpKeyProvider("https://x", store)
	req, _ := http.NewRequest("GET", "https://x", nil)
	if err := p.Inject(req); err == nil {
		t.Error("AQP Inject with no SSO cookie file: want error, got nil")
	}
}
...
// --- gateway: newAqpClient + GetManagedKey cache + InvalidateKey ---

func TestAqpClient_GetManagedKeyCache(t *testing.T) {
	dir := t.TempDir()
	store := filepath.Join(dir, "aqp_oauth_auth.json")
	os.WriteFile(store, mustMarshalT(map[string]string{"sso_session_cookie": "SSO_C=abc"}), 0o600)
...
func TestAqpClient_GetManagedKey_NoCookie(t *testing.T) {
	c := newAqpClient(filepath.Join(t.TempDir(), "missing.json"))
...
func TestAqpClient_MonthlyUsage(t *testing.T) {
	dir := t.TempDir()
	store := filepath.Join(dir, "aqp_oauth_auth.json")
	...
	// MonthlyUsage() uses the package-level aqpMonthlyUsage constant, so we
...
func TestAqpClient_MonthlyUsage_NotLoggedIn(t *testing.T) {
	c := newAqpClient(filepath.Join(t.TempDir(), "missing.json"))
