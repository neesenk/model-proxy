...
- **reasoning effort 方言（`internal/provider.ChatReasoningMode` → `protocol.RequestOptions.ReasoningDialect`，cc-switch mapReasoningEffort）**：r→chat 的 `reasoning.effort` 按 provider 渲染——zhipu/volcengine/kimi-code/deepseek → `thinking:{type:"enabled"|"disabled"}`；qwen-plan → `enable_thinking: bool`（仅 none 为 false）；aqp、shopee（OpenRouter 系）→ 原生 `reasoning:{effort}` 对象；其他 → `reasoning_effort` 原样。无 reasoning 字段时任何模式都不发声。**effort 枚举层（`internal/provider.ChatEffortProfile` → `RequestOptions.ReasoningEffortEnum`/`ReasoningEffortOnly`）**：chat 端点接受非透传 effort 枚举的 provider 在 switch 之上追加映射后的 `reasoning_effort`（deepseek low|high|max；zhipu glm-5.3 none→low/仅 low|high|max、glm-5.2 全枚举；qwen-plan qwen3.8 xhigh|medium|low）；`EnumOnly`（kimi-k3，thinking+reasoning_effort 同发会被拒）时枚举**替代** thinking switch。有枚举时 minimal 映射到真实 low 档、switch 保持 enabled（legacy 的 none/minimal→disabled 只在无枚举时保留）。**未知非空 effort（如 codex "persistent"）先归一化为 high** + `unknown_effort` 诊断（业界 clamp-down 约定），r→chat/r→a/chat→a 三处调用点统一归一。
