...
- `messages` ↔ `input` items：text block ↔ `{type:message, content:[{input_text|output_text}]}`；image ↔ `{input_image, image_url}`（base64 转 data URL，url-source 原样保留；chat→r 保留 `image_url.detail`）；Anthropic `document` ↔ Responses `input_file`（base64 data URL/http(s) URL/filename），Chat 使用 `file` part，URL-only Chat 降级为带文件名的文本链接。**file_id 不跨协议透传**：file_id 是 provider 域作用域的引用，跨协议（几乎必然跨 provider）的目标不认识它——六个方向统一处理：仅 file_id 时整个附件降级为可观测的文本注记 + `convertWarn`（`file_id_degraded` 诊断）；与 file_url/file_data 同时出现时保留可传输的 inline base64/URL 源、仅丢弃 file_id 并发同一诊断（r→chat/r→a 同样偏好 URL 源；Switchyard 同款语义，见 `docs/decisions/intentional-behaviors.md`）。**无 type 的 message 简写项**：Responses API 允许 input 里的 message 省略 `type`（`{role, content}` 简写，pi-ai/openai-responses 即此形态）——`responsesInputItems` 统一归一化为 `type:"message"`（有 `role` 且无 `type` 时补键），否则 r→chat/r→a 会把它当未知 item 丢弃、上游收到空消息列表（zhipu 400 "输入不能为空" 1214）；message content 的字符串简写同样合法，r→chat 经 `chatContentText`、r→a 在 `responsesContentToAnthropicBlocks` 塌缩为单个 text 块。
- anthropic `tool_use` / chat `tool_calls` → responses `{type:function_call, name, arguments, call_id}`；tool `id` ↔ `call_id`（`function_call_output` 的 `call_id` 必须回填）。arguments 缺省补 `"{}"`；chat→r 对只带 id 不带 name 的 tool_call（replace-style 客户端重发）按 call_id 从同请求前面的 function_call 回填 name，仍缺则 convertWarn。**legacy chat 形态**：chat→r 把 name+content 齐备的 `role:"function"` 消息映射为 function_call_output，call_id 按 name 从较前 function_call 回填（`pairLegacyFunctionOutputs`），无法配对丢弃 + convertWarn；assistant legacy `function_call`（无 tool_calls 数组时）映射为 function_call item（合成 call_id `legacy_fc_<name>`）；请求级 `functions`/`function_call` 在 chat→a 与 chat→r 均丢弃 + convertWarn——legacy `role:"function"` 不泄漏进 Responses input。
...
- `max_tokens` ↔ `max_output_tokens`；chat `max_completion_tokens` 与 `max_tokens` 并存时取前者（chat→a、chat→r 均同）。`thinking.budget_tokens` ↔ `reasoning.effort`（best-effort 近似）。effort→budget 阶梯覆盖全枚举并对齐真实客户端 budget 聚类（pi 1024/2048/8192/16384、opencode 16000、kimi 32000）：minimal=1024、low=2048、medium=8192、high=16384、xhigh=32000、max=顶格 max_tokens−1，none 不发声、未知非空值 clamp 到 high（16384）；随后钳制为 min（阶梯值, max_tokens−1)——Anthropic 要求 1024 ≤ budget_tokens < max_tokens，有效 max_tokens ≤ 1024 时 thinking 无法合法表达、不发声；r→a 与 chat→a 两个调用点均传入注入默认 4096 后的有效 max_tokens，chat→a 显式 `"max_completion_tokens":null`/`"max_tokens":null` 视为缺失。反向 budget→effort 阈值取阶梯值（≥32000 xhigh、≥16384 high、≥8192 medium、≥2048 low、>0 minimal，enabled 无 budget 归 medium），阶梯各档正反向互逆（未钳制时）。chat `reasoning_effort` ↔ responses `reasoning.effort`（chat→a 也映射到 `thinking`；**chat→r 同 a→r 一样恒带 `summary:"auto"`**，否则 codex 不回 reasoning summary）。r→chat 输出侧只写 `max_tokens`（兼容性最好）。**r→a 缺失 `max_output_tokens`（或为显式 null，codex 客户端常态）时注入与 chat→a 相同的默认值 4096**——anthropic 对缺 max_tokens 必 400。
- chat `response_format` ↔ responses `text.format`（`json_object` 直通；`json_schema` 拆装一层 `json_schema` 包装；**空包装（无 schema）丢弃 + convertWarn**——裸 `{"type":"json_schema"}` 上游必 400；嵌套 `type` 不覆盖判别式）；anthropic 无对应，r→a 与 chat→a 均丢弃 + `response_format_dropped` 诊断。
...
- r→a 与 chat→a 一样保证首消息为 user（input 以 function_call 开头时插入占位 user）。**空块守卫**：r→a 与 chat→a 跳过空 text part（Anthropic text block minLength 1），不产生空 blocks 数组/`content:null` 消息（len(blocks)>0 守卫）；r→a 的 function_call_output parts 数组在文本为空时不再产出前导空 text 块；summary 为空且无 encrypted_content 的 reasoning item 丢弃 + 告警（`reasoning_dropped`），不再合成空 thinking 块。
...
- `response.created` → anthropic `message_start` / chat 首 chunk（`role:assistant`）。r→chat 的每个 chunk 必带 `created`（response.created/completed 的 `created_at` 优先，否则取流开始时刻，整条流恒定）与 `model`；非流式 chat.completion 同样把 `created_at` 映射为 `created`。
...
- `message_start`/首 chunk → `response.created`；content 块增量组装回 `output_item.added` + 对应 `*.delta` + `output_item.done`（added/done 按 item id + type 严格配对；从未发过 added 的空 text/thinking 块不发任何 done 帧）。**`output_item.done` 帧与 `response.completed.output` 携带完整 item**（message 带 content、function_call 带 name/call_id/arguments、reasoning 带 summary/encrypted_content、custom_tool_call 带 input，对齐真实上游 framing，cc-switch streaming_codex_chat 同款）。**content_part.added/done、output_text.delta/done、reasoning_summary_text.delta/done 帧均带 `item_id`**；reasoning_summary_part.added/done 携带 `part:{type:"summary_text",text}` + `summary_index`（规范形态，不再塞整个 item）；`reasoning_summary_text.done` 带完整累计文本，r→{a,chat} 消费者识别 `response.reasoning_summary_text.done`。
...
- `message_stop`/finish → `response.completed`（带 usage）；status 为 `incomplete` 时事件名为 `response.incomplete` 并带 `incomplete_details.reason`（`max_tokens`/`length`/`model_context_window_exceeded`→`max_output_tokens`，`refusal`/`content_filter`→`content_filter`）。**每个合成帧带递增 `sequence_number`（从 0 开始每帧 +1，对齐真实上游 framing；JSON→SSE 桥 `responsesJSONToSSE` 同样逐帧打号）。** **合成的 completed/incomplete 快照带 `created_at`（Unix 秒整数，Responses 线上契约；JSON→SSE 桥在构建 `response.created` 前回填，created 与 completed 快照同值）**——严格 SDK 解析必填。**非流式合成 Responses 对象同等级补齐规范必填键**（`responsesRequiredKeys`：`created_at`、`"error":null`、`"incomplete_details":null`（无 reason 时）、`"tools":[]`、`"tool_choice":"auto"`、`"parallel_tool_calls":false`、`"metadata":{}`），合成 output item 带流式同款 `msg_item_<n>`/`rs_item_<n>`/`fc_item_<n>` id，output_text part 恒带 `"annotations"`（无引用时为空数组）。JSON→SSE 桥的 content_part/output_text 帧带 `item_id`，reasoning summary 帧用规范形态（summary_part.added/done 带 `part` + `item_id` + `summary_index`，summary_text.delta 配对带全文的 .done）；anthropic JSON→SSE 桥在源 JSON 无 usage 时省略 message_delta 的 usage 键（不发 `"usage":null`）。
...
- 跨协议 HTTP 4xx 不进入成功响应转换器：`convertErrorResponse` 保留 status、message、code/param 与 request_id，并翻译为客户端错误 envelope；无 `error` 键的裸 `{"type":"error",...}` 信封按顶层字段转换（字面 `"error"` 不当错误类型，回退 status 推导）；FastAPI 风格 `{"detail":...}`（codex/ChatGPT 后端）按 detail 作为 message 转换。**无法解析或无法识别的错误体绝不使转换失败**：合成为携带截断原文（≤500 字节）或 status 兜底文案的目标协议 envelope——失败关闭会把可转述的 400 升级成掩盖上游诊断的 "response conversion failed" 502。Responses 非流式 `status:"failed"|"cancelled"` 即使缺少 `error` 对象也必须转换失败，不得合成空成功响应。
- 响应 `reasoning_content` ↔ thinking block（非流式置于 text 前；流式 `thinking_delta` 独立 block 生命周期；多个 thinking 块合并为 reasoning_content 时以 `\n\n` 分隔）。stop 语义：`content_filter ↔ refusal`；`pause_turn`→`stop`（best-effort）。a→chat 非流式合成 chat.completion 带 `created`（Unix 秒）；chat→a 合成响应恒带 `"stop_sequence"` 键（无则 null，与流式路径一致）。
...
- 空 arguments 补 `{}`；
- finished gate 防止重复 finish；每个 chunk 带 `created`（流开始时刻，整条流恒定）与 `model`；finish chunk 只带 finish_reason，usage 独立成空 `choices:[]` chunk 紧随其后（include_usage 契约），再发 `data: [DONE]`；finish 后的 content/tool 帧被 post-terminal guard 抑制；
- error event 转成 OpenAI error chunk，其后不补 usage chunk / `[DONE]`（fail-closed，与 r→chat 一致）。

## Tool ID

OpenAI/Responses → Anthropic 的 tool use id 必须满足 `^[a-zA-Z0-9_-]+$`（r→a 的 call_id 是自由形态如 `call.a`，请求、非流式响应与流式三路径共用同一清洗语义）。同一调用内 tool_use/tool_result 使用 per-conversion memo 保持成对映射；纯函数映射必须确定性。**同一请求内两个不同原始 id 清洗后碰撞**（如 `call.a` 与 `call_a` 都得 `call_a`）时，按出现顺序加 `_2`/`_3` 后缀去重，保持 use/result 配对（Switchyard FNV-1a 后缀解决同一问题）。**无 id 的 tool_call 不共享占位符**：每次出现分配新的 `toolu_empty_N`，无 id 的 tool_result 按出现顺序与这些占位符位置配对（chat→a 与 r→a 同规则）。

可选字段（call_id/name/arguments/model/id/stop_reason 等）缺失时**绝不产生字面量 `"null"`**——取值统一用 `strOpt`/`strKey`（`strOf(nil)` 会渲染成 `"null"` 字符串并击穿 `firstNonEmpty` 回退）。缺失时的具体行为：function_call `arguments` 缺省 `"{}"`（空串会让下游 `JSON.parse("")` 失败；r→chat 非流式响应路径同样遵守，不再渲染字面 `"null"`）；chat→r 请求里 `name` 缺失时按 `call_id` 从同请求较前项回填（replace-style 客户端只回传 id）并 `convertWarn`；流式合成缺失 tool id 时回退到合成的 item id（`fc_item_<idx>`）。

## Usage

- Anthropic → OpenAI 请求注入 `stream_options.include_usage`；r→chat 的流式请求同样注入（kimi/MiniMax 类上游否则流式 usage 全 0）。下发侧遵守同一契约：finish chunk 只带 finish_reason，usage 独立成空 `choices:[]` chunk 紧随其后，然后 `data: [DONE]`（a→chat、r→chat 同形，干净流上无条件下发）。
...
SSE 读取按规范折叠多行 `data:`（连续 `data:` 行在分派空行处以 `\n` 拼接为一个 payload，六个流式转换器与 `parseWireSSE` 聚合路径一致；无尾空行的尾帧在 EOF 分派）。LLM 上游实践全部单行，多行仅出现在方言网关。对漏写帧间空行、导致多个完整 JSON payload 被折叠到同一帧的非规范网关，解析器逐 data 行恢复；每个恢复帧使用该 data 行读取时对应的 `event:`，不能把最后一个 event 套给全部 payload，遇到协议终态后不再处理同一折叠帧中的后续数据。聚合路径 `parseWireSSE` 与流式 reader 同款逐行恢复（恢复时 convertWarn；恢复出的字面 `[DONE]` 行是显式终止符，其后行不再处理）。anthropic↔chat 流式 reader 额外：容忍流首一个 UTF-8 BOM；折叠帧中恢复出的 `[DONE]` 行判为终止符；chat→anthropic reader 识别 `event: error` 帧（payload 无 error 键时经 `chatSSEErrorOf` 提取 message/detail，cc-switch extract_chat_sse_error）。
...
`unknown_effort`（未知非空 reasoning effort clamp 到 high——strict 模式同样拒绝）。
...
- **响应 model 归一化（provider alias）**：target 真实模型与 calledModel（暴露名）不一致时，executor 在 commit 前把客户端可见字节里的 model 改写回 calledModel（`internal/protocol/model_normalize.go` 的 `NormalizeResponseModel` / `NormalizeSSEModelStream`）。覆盖每条客户端路径：非流式 JSON 顶层 model（透传与转换后的字节统一后处理）、openai SSE 每 chunk 顶层 model、anthropic `message_start` 的嵌套 `message.model`（改写后整流快进透传）、responses 快照的 `response.model`。改写是结构化 splice（解析 JSON token 流后只替换 model 值），绝不裸字符串替换——生成内容里的 `"model":"x"` 字面量不受影响，未命中改写的字节原样保留。归一化是 best-effort：只改写上游**已给出**的 model 值，上游没返回 model 字段（或非 JSON body）时字节原样透传，绝不凭空注入；只有 body 读取失败与转换读取失败一致 fail-closed（502）。归一化位于 logger/usage/cache **内层**，因此 cache 存储与命中重放的都是归一化后的客户端字节（cache key 语义不变）；target == calledModel 时归一化完全不启用，零拷贝透传字节级不变。请求方向行为不变（`Plan.RewriteModel` 仍改写上更名）。Shadow 不回客户端，不改写；Fusion 合成响应经同一 executor 路径自动覆盖。
...
- reasoning effort 枚举层与阶梯（`internal/protocol/convert_effort_profile_test.go`、`internal/provider/reasoning_effort_test.go`、`internal/targetexec/plan_test.go` 的 `TestPlanConvertBodyThreadsEffortProfile`）：`ChatEffortProfile` 各 provider 枚举映射与 model 门控（kimi-k3 vs k2.x、glm-5.3 vs 5.2 vs 4.6、qwen3.8）、thinking/enable_thinking 方言 + 枚举渲染、`EnumOnly` 替代 switch、未知 effort clamp high + `unknown_effort` 诊断（r→chat/r→a/chat→a 三调用点）、nil-Enum 纯 switch 回归；阶梯值与全 rung 断言见 `TestConvertReasoning_EffortMapping`/`TestSpecFix_EffortLadderAllRungs`。
...
- responses 方向 P1 修复：a→r `reasoning.summary:"auto"`（chat→r 的 `reasoning_effort` 映射同样恒带 `summary:"auto"`）+ codex `include:["reasoning.encrypted_content"]`（后者在 `internal/provider/provider_usecase_test.go`）、reasoning 方言穷举（reasoning 对象/reasoning_details，流式+非流式）、r→chat reasoning user 边界回溯、合成流 done/completed 帧完整 item + 空块无 done、r→a cache_write 双减、r→a tool 输出 parts 数组拆块、流式 incomplete 读 usage、流式 completed cancelled/error fail-closed、refusal 正文保留、tool_use 缺 input arguments 兜底 `"{}"`、chat→r 流式 `event: error` 判错。
