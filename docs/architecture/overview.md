...
`routing.NativeProtocols` 的静态原生协议覆盖度做 unified/split 两种写入
（单协议归一 vs 按原生协议拆分配置项，让 agent 尽量走字节级透传），判定信号
除 config 声明外还以只读方式消费 daemon 的逐模型协议探测缓存（`model_caps.json`，
经指纹校验防陈旧）；其他输入只有 config DTO、catalog 元数据与
