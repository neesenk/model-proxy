...
- **UA 构成（3.14.0，2026-09-23 真实 CLI 抓包实测）**：`ZCode/3.14.0 ai-sdk/provider-utils/4.0.27 runtime/node.js/24`，**没有 `ai-sdk/anthropic/<ver>` 段**。机制：`@ai-sdk/anthropic@3.0.81` 的 `getHeaders()` 确实会追加该段，但 ZCode 在 per-request header 层（`runner-options.ts` `mergeRequestHeaders`）用 defaultHeaders 里的 `User-Agent: ZCode/<ver>` 覆盖掉它，provider-utils 随后才追加 `ai-sdk/provider-utils/<ver>` + runtime 段。Node≥21.1（产品 `.nvmrc` 24.14.0、engines≥24）runtime 走 `navigator.userAgent` 分支 → `runtime/node.js/<major>`，故产品形态发 `/24`（本地用 node 26 跑则 `/26`）。**这是"读源码推断被实测推翻"的一条**：曾按 SDK dist 代码给 UA 加过 `ai-sdk/anthropic/3.0.81` 段，抓包后已移除。
...
- **复测清单（开源后待办）**：① 用真 coding-plan key 对直连 `open.bigmodel.cn/api/anthropic/v1/messages` 跑 messages + `quota/limit`，确认套餐系数/优先级仍在；② 若失效，评估把 `anthropic_base_url` 切到 `https://zcode.z.ai/api/v1/ultra/anthropic`（指纹与鉴权头原样透传，仅换 base）；③ ~~复抓 3.14.0 核对 UA 是否含 `ai-sdk/anthropic/` 段~~ **已完成（2026-09-23，见下"自动差分测试"）**：结论是不含该段，代理已按实测修正。
- **自动差分测试（2026-09-23 建成）**：`scripts/zcode-wire-diff/` 从开源仓库构建真实 ZCode CLI，经个人 provider 配置把模型请求打到本机 capture server（不触碰 BigModel、不需要真 key），产出 golden fixture `internal/provider/testdata/zcode-wire/real-zcode-cli.json`；`internal/provider/zcode_wire_test.go`（`TestZCode_WireMatchesRealClientCapture`）逐头比对：确定性值精确匹配、UUID 头形状匹配、**头集合 parity**（真实客户端发了而我们没发的、或我们多发的，测试即红）。首次捕获（v3.14.0，28 头）与代理指纹完全一致，并推翻了"UA 含 `ai-sdk/anthropic/` 段"的源码推断（见上）。ZCode 发新版本后跑 `scripts/zcode-wire-diff/run.sh` 即可刷新 fixture 并 review diff。
...
- **计费：仅按量付费，且无 API key 可用的余额接口**（2026-09-24 实测）：社区流传的 `GET /api/v1/balance` + `Authorization: Bearer <key>` **不成立**——`platform.xiaomimimo.com/api/v1/balance`（正确 host）对每种鉴权头拼写（Bearer/api-key/X-API-Key/token）都回 `{"code":401,"loginUrl":"https://account.xiaomi.com/pass/serviceLogin…"}`，即该端点挂浏览器 SSO cookie（`serviceToken`）；同路径在推理 host `api.xiaomimimo.com` 上则 404（openresty）。cc-switch issue #2488 同结论（“小米没给用 token 查余额的 api”），其可用方案是从控制台页面复制 cookie。**因此不配 `usage_url`**：`login mimo` 回退 `openai_base_url/models`（Bearer GET，401/403 拒——实测该端点认 key）验 key；`Quota()` 返回 `BillingUnknown` + `RemainingPct=-1` + 控制台 URL Notes（qwen-plan/step-plan 模式，surplus 调度按 priority 中性排序）。`billing: pay-as-you-go` 照常设置——它是**付费方式元数据，不门禁任何东西**：配额轮询看 provider 实现（配了 `usage_url` 就查，否则用实现自己的 console-only snapshot），展示看 snapshot 的 Notes，调度只看实测 snapshot 的 BillingClass。三者都不读 config 的 `billing:` 标签（曾错误地门禁轮询/覆盖实测 tier，2026-09-24 修正，见 `docs/decisions/intentional-behaviors.md`）。
- **有意不抓控制台 cookie**：与 qwen-plan 同决定（`docs/decisions/intentional-behaviors.md`）——尊重平台条款，也不把浏览器会话搬进代理。余额请到控制台 `https://platform.xiaomimimo.com/#/console/balance` 看（CLI `usage mimo` 与 Web UI Notes 均给该链接）。
- **Token Plan 有意不支持**：套餐用量端点（`platform.xiaomimimo.com/api/v1/tokenPlan/usage`，响应形状据 cc-switch 自定义数据源为 `{data:{usage:{items:[{name:"plan_total_token",used,limit}],percent}}}`）同样 cookie 鉴权；订阅账号在本代理下按按量呈现，耗尽 reactive failover（见下）。
...
- 耗尽：402 + `error.metadata.limit_source`（`openrouter_credits`/`openrouter_key_limit`/`openrouter_in_flight_budget`）或 "Not enough credits" 文案 → targetexec body-proven quota-denied（marker 表 `limit_source`/`not enough credits`）覆盖 402/403 → quota 冷却 + failover。`/models` 公开且忽略鉴权（invalid Bearer 也 200），login 校验走 usage_url `/key`（真 401），且无鉴权的 `/models` **不作为** login 的 usage-拒绝回退（`provider.ModelsAuthless`，opencode-go 同列）——否则任意坏 key 都会被回退放行进池。
