...
| DELETE | `/api/security/blocks/<session-id>` | — | `{status:"unblocked",session_id}` | 解除一个会话拉黑：立即恢复放行该会话请求（`model-proxy guard unblock` 的 WebUI/CLI 同源端点）。路径缺 session id → 400；会话不在表中 → 404（后端 message） |
