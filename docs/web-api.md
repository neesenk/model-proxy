...
| POST | `/api/presets/<name>` | — | `{status:"added",warnings:[模型名…],reload_warning?:"…"}` / 400 | 合并模板 provider 块到 config.yaml（幂等：块已存在则跳过合并）→ 与 `/api/config/edit` 同管线 validate+saveAndReload；`warnings` 只含隐式路由歧义模型名（该模型同时被其他已配置 provider 服务且无显式 route），UI 需展示；reload 失败时块已落盘但 daemon 仍是旧 generation，错误串放 `reload_warning`（与歧义模型名分字段，UI 不得当作模型名渲染）；凭据随后走 `/api/accounts/<provider>` 或 `/api/login/<provider>/start`；未知 preset 400 |
