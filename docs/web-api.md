...
| GET | `/api/security/adjudications` | — | `{adjudications:[{ts,kind,rule,verdict,reason?,model?,request_id?,session_id?,action?,cached?}…],stats:{calls,input_tokens,output_tokens}}` | guard AI 二次判定（`guard.adjudicate`）的最近 verdict ring（内存，上限 256，新到旧）：high/low/error/skipped；low 是被屏蔽的命中（无审计记录，此处可见）；`reason` 是经 scrub（控制字符过滤 + 命中片段回显掩码）与限长的模型解释。通道未配置 → 空列表（fail-soft）。`stats` 是判定通道的 LLM 用量记账：真实模型调用次数与 token 合计（in/out）——缓存命中与在途去重不计费（判定调用走 provider 直连、有意不进 forward 统计，这是它自己的用量面） |
