...
| GET | `/api/security?kind=&from=&to=&limit=` | — | `{enabled,records:[{ts,kind,request_id?,agent?,protocol?,exposed?,names?,action?,verdict?,detail?}…],skipped}` | 安全审计日志查询（`internal/observe/seclog`，guard 命中的持久化记录）。`kind` 仅 `secret\|path\|drift`（其他值 400）；`from`/`to` 解析惯例同 `/api/stats`（unix 秒或 RFC3339，inclusive，内部转成审计日志的 unix 毫秒）；`limit` 默认 100 上限 1000，records 按 ts 新到旧；`skipped` 是扫描时跳过的不可读行数。审计目录由 `guard.audit_path`（默认 `~/.model-proxy/log/security/security.log`）的目录派生；`guard.audit: false` 或目录不存在 → `{enabled:false,records:[],skipped:0}`（惯例同 request_log 关闭）。**records 只含模式/路径类别名与动作，绝不含命中内容**（seclog 红线）。`verdict` 非空表示该记录来自（或 fail-open 自）AI 二次判定通道（high/low/error/skipped；low 记录是"判为良性后的一次性留痕"，同一内容缓存命中不重复落盘）。path 的 weak 命中（正文/工具结果侧地址提及）完全忽略——不计数、不落审计（decision 23）；历史日志里的 action=`log-weak` 记录是调整前写入的 |
| GET | `/api/security/explain?request_id=&kind=&name=` | — | `{status,request_id,kind,matches:[{name,strength?,regex?,source?,explanation?,pre?,hit?,post?,located}…],adjudications?:[{rule,verdict,reason?,model?,ts?,cached?}…]} ` | guard 命中的**按需实时分析**（UI Security 页行内 analyze）：按 `request_id` 从 request_log 拉取该请求的 body，用**当前代** guard scanner 重扫定位每处命中。`kind` 仅 `secret\|path`（drift 无请求体 → 400）；`name` 逗号分隔、必填（即审计记录的 names）；参数缺失/非法 → 400。`status` 枚举：`ok`；`no_request_log`(request_log 关闭）；`not_found`（记录已被 retention 清理）；`redacted`(guard.secrets=redact 时日志存的是打码后 body，命中原文已销毁）；`cross_request`（仅 `known_secret_fragmented`：跨请求会话态检测，单条 body 无法重定位）;`scanner_unavailable`（当前代无 scanner，fail-closed)。matches 每处命中一条（每 name 上限 5):`strength` 仅 path 有（`strong`=工具调用侧 tool_use.input/function arguments，`weak`=普通文本与工具结果侧内容，结构识别失败一律 weak);命中上下文按 `pre`/`hit`/`post` 三段预切分返回（~80 字节窗口；**不用字节偏移**——前端 JS 按 UTF-16 切字符串，偏移跨编码不可靠）；**secret 命中段掩码为头4+…+尾2（<8 字节全掩），且窗口内任何其它 secret 命中也一律掩码**（path 命中的上下文可能夹带无关密钥，body 整体按敏感处理），path 字面量原文展示（路径非凭据）；内置/自定义规则带 `regex`+`source`，known_secret*/path 类别带静态 `explanation`;`located:false` 表示当前 scanner 重扫未命中（规则已删/凭据已轮换），仍带解释。`adjudications` 是该请求已记录的 LLM 判定结果（审计日志 verdict 记录为持久源，仍驻留 ring 的条目补全 model/cached 归属）——analyze 视图在定位命中旁直接展示判定结论。不脱敏不落盘例外边界见 decisions（命中内容仍不入审计日志；本端点派生自 admin 本可读的 request_log，不产生第二份持久副本） |
| GET | `/api/security/adjudications` | — | `{adjudications:[{ts,kind,rule,verdict,reason?,model?,request_id?,session_id?,action?,cached?}…],stats:{calls,input_tokens,output_tokens},enabled}` | guard AI 二次判定（`guard.adjudicate`）的最近 verdict ring（内存，上限 256，新到旧）：high/low/error/skipped；low 是被屏蔽的命中（无审计记录，此处可见）；`reason` 是经 scrub（控制字符过滤 + 命中片段回显掩码）与限长的模型解释。通道未配置 → 空列表（fail-soft）。`stats` 是判定通道的 LLM 用量记账：真实模型调用次数与 token 合计（in/out）——缓存命中与在途去重不计费（判定调用走 provider 直连、有意不进 forward 统计，这是它自己的用量面）。`enabled` 是当前代 `guard.adjudicate` 开关（Security 页 Rule hits 卡的降噪提示依据，区分“关闭”与“安静”） |
| GET | `/api/security/blocks` | — | `{blocks:[{session_id,kind,rule,reason?,model?,request_id?,ts}…]}` | guard.adjudicate 高 verdict 拉黑的会话表（持久化 `guard_blocks.json`，跨重启保留，直到显式解除），新到旧 |
