...
+ granularity 分段钮 auto/minute/hour/day/week/month，**与窗口联动**：选项按窗口跨度启用/
禁用（分钟≤2d、时>1h 且≤31d、日>2d 且≤400d、周>7d、月>14d；小时窗口只剩分钟），
auto 按跨度选分钟≤6h/时≤7d/日≤90d/周，存住的显式选择被新窗口禁用时回落 auto；**应用新时间范围时粒度重置回 auto**——
分钟粒度对新月宽毫无意义（`analyticsGranOptions`/`analyticsGranularity`，pure.js）+ provider/model
datalist 过滤 + by model/agent 维度切换；**默认 Last 1h**；选 Last 1h/Today 时**自动刷新**（30s，仅当 Analytics 为激活页且日期选择器关闭时））→ **KPI 行**（tokens=四桶直合（in+out+cache_creation+cache_read，同 Status→Token usage 口径，tooltip 给四桶分解）/
**tok/s** / **cache hit** / requests / failures / cost 六张 chip（顺序：tokens→tok/s→cache hit→requests→failures→cost）（tok/s 直读服务端统一字段 `totals.tok_sec`——output ÷ 完整调用时长，解码速度；tokens/tok/s/cache hit 全部来自统一派生块，前端零公式），各带等长前窗 Δ%；hit = cache_read/(input+cache_creation+cache_read)；未定价模型数标在 cost chip 上）→
**单张趋势图**（uPlot，metric 分段切换 tokens/toksec/cache/latency/ttft/requests/errors/cost（tok/s 直读 per-point 服务端字段 `tok_sec`（output ÷ 完整调用时长秒，分母非响应头 latency）；tokens 指标 = 服务端四桶直合 `tokens` 字段）；
...
$/1M tok/cost share 比例条，按当前 metric 排序）。per-(provider,model) 的 token/请求
