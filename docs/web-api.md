...
| POST | `/api/presets/<name>` | — | `{status:"added",warnings:[模型名…]}` / 400 | 合并模板 provider 块到 config.yaml（幂等：块已存在则跳过合并）→ 与 `/api/config/edit` 同管线 validate+saveAndReload；`warnings` 是隐式路由歧义（该模型同时被其他已配置 provider 服务且无显式 route），UI 需展示；凭据随后走 `/api/accounts/<provider>` 或 `/api/login/<provider>/start`；未知 preset 400 |
