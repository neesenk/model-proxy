...
13. **Responses 跨协议 body 解析失败落 502 而非 400**：responses 客户端走跨协议 target 时 body 先经 `responsesState.expand` 做本地历史展开（`internal/forward` 的 serveOnce）；body 本身是非法 JSON 时展开报错、当前 target 被跳过，所有 target 都失败后走统一的 all-targets-failed 路径返回 502，而不是 400。转发主路径不整体解析客户端 body（同协议字节透传），调度层无法区分「客户端 body 坏」和「单 target 处理失败」，保持 fail-closed 跳过语义。
...
20. **guard 分片检测的会话窗口存 redact 前原文**：`guard.session_scan` 的会话窗口（store 由 `internal/guard/session` 拥有；按 `x-claude-code-session-id`，每会话 32KiB 尾窗、LRU 256、≤8MiB）保存的是 redact 之前的请求 body——存 redact 后形态会让后续分片检测失效（被抹掉的分段永远拼不回来）。这是内存敏感性的有意取舍：窗口可能含凭据，因此只活在小锁保护的进程内存里，永不落盘/日志/序列化/API；有界性（截断即重置分片进度、LRU 整体淘汰）是它的暴露上限。它不进 RuntimeSnapshot（跨代运行时观察态，参照 metricsStore），reload 不清空，避免 reload 抹掉在途会话的分片上下文。会话键是**客户端可控**的 `x-claude-code-session-id` 头：省略或轮转该头的 agent 永远不会进入本通道，256 个不同 id 即可挤空整个 LRU——它是对配合客户端的有界启发式，不是对有意规避者的检测保证。body 中 `client_metadata.session_id` 仅用于观测与 adjudication job 的 session 维度（见决策 36），不进入 block / session_scan / repeat 拦截等安全决策。
21. **guard 分片命中＝精确匹配拦截（决策 37 语义；历史上曾按 action 降级）**：分片泄露的秘密横跨多条请求，没有任何单个 body 可以被改写——拼装完成（`known_secret_fragmented`）现在一律记录 + 拦截补齐请求 + 拉黑会话（与所有精确匹配同族，不再按 `guard.secrets` 档位区分；旧版 redact 曾降级为 log、block 只拦补齐段）；此前的分段已放行，因为它们各自是不含完整秘密的干净请求，单请求扫描无从拦截。检测有两条通道（`internal/forward/forward.go` 同处运行，本条是唯一权威定义）：**① 相邻窗口精确拼接**——只在"拼接体命中而尾窗单独不命中、当前 body 单独不命中"时计 fragmented；一个只存在于拼接体的命中必然跨越接缝，而 known-secret needle 最长为 `Scanner.MaxKnownNeedleLen`，因此实现只扫描接缝区（尾窗末尾 + 当前 body 开头各 MaxKnownNeedleLen-1 字节），判定与整段拼接扫描完全等价，但省掉每请求至多 64MiB+32KiB 的拼接拷贝；尾窗单独命中判定来自会话条目里 Add 时增量维护的缓存（截断或换代失效时回退重扫），不再每请求重扫尾窗。真实流量上本通道接近零命中（能转发的 body 都以 `{` 开头，ExtractModel 要求，两条 JSON 请求的分段在拼接处永远不可能字节相邻），但作为 fail-closed 兜底保留。**② 按序最长前缀进度**（`ScanKnownFragment`，每段 ≥8 字节、只覆盖 known-secret 原文）——覆盖分段各自落在单条请求 JSON 内部的真实拆分，是实际起作用的通道。某秘密跨请求补齐（或单请求出现完整值）时扫描器将其进度**刻意归零**并经 `reset` 返回值声明，存储层合并进度时对恰好这些下标取 0 而非元素级 max——否则陈旧进度会让后续 suffix 分段把同一秘密重复报成 fragmented（回归测试：`TestSessionScan_NoRefireAfterCompletion`）。
...
30b. **跨协议错误体降级不 fail-closed**：`convertErrorResponse` 对无法识别/无法解析的上游错误体（FastAPI `{"detail":...}`、HTML 错误页、空 body）**不**返回错误把 4xx 升级成 "response conversion failed" 502，而是合成携带截断原文（≤500 字节）的目标协议 envelope——客户端必须能看到上游的诊断原文。同协议（如 responses→responses）仍字节透传。锁定测试：`TestConvertErrorResponseDegradesUnrecognizedBodies`。
...
36. **guard AI 二次判定（`guard.adjudicate`）：内容外发的显式 opt-in 例外 + 三级判定 + 全链 fail-open + 持久化拦截**：pattern 命中（规则表/自定义/编码通道，非 known-secret 精确通道）与 strong 路径命中（paths=log 档）异步判定——命中片段（±`context_bytes` 上下文，窗口内**其它** secret 命中先按 explain 同款掩码）发给 `guard.adjudicate.model` 指定的模型。例外成立条件：① 默认 off，模型由用户显式指定；② 判定调用经**共享调度 seam**（internal/app/model_call.go 的 modelExchange：与 forward 同一条 Manager.DecideOrder 排序 + 冷却/熔断/模型锁目标跳过 + pin 生效，judge 用独立 sticky 槽 `guard-adjudicate:<model>`；每次 attempt 独立切预算——首 attempt 拿剩余大头（慢而健康的判定模型能完成判定），后续 attempt 各保留 min(4s, 均分值) 份额，单个挂起/劣化目标吃不光整个预算；判定请求显式 `thinking disabled`——回复是短 JSON，扩展思考纯属延迟，实测 flash 档 15s → 2s），成败/429 回写共享健康状态（429 经 targetexec.ParseRateLimit 同一分类入口；外层预算耗尽等价 client-cancel 不计数）；传输仍是 probe.Do 配方、**不进 forward transport 层**（自我递归触发 guard / 污染 request log/cache/stats）；全部目标冷却中 → 快速失败走 fail-open。判定用量（calls/in/out tokens）与累计 low 计数持久化在 `guard_stats.json`（重启累计不清零；low 计数覆盖缓存回放，行仍 ring-only）；KPI verdict 计数由服务端对 SQLite 聚合（`/api/security` 的 `counts` 字段，窗口跟随查询），不再客户端数合并流——合并流含 ring 缓存重放、重启即漂移；③ 发送的是最小片段而非整个 body；④ 模型回复的 reason（判定逻辑 ≤200 字符）与 evidence（事实依据 ≤300 字符）都经 scrub（控制字符过滤 + 命中片段回显替换为 `[MASKED]`）后才进审计/ring。判定命中后的 session block / repeat 拦截 / 会话分片检测只使用受信请求头 `x-claude-code-session-id`；body 中 `client_metadata.session_id` 仅用于观测与 adjudication job 的 session 维度，不进入安全决策。判定语义（三级）：**high**＝真实可用凭据外传/工具调用真实读取凭据文件 → 经典审计记录（`verdict=high`）+ live event + `block_session` 时拉黑会话；**medium**＝形似风险但窗口内无法确证（触碰凭据路径的工具调用、歧义操作语境）→ 只落 `verdict=medium` 审计记录，不拉黑、不发 live event；**low**＝占位符/夹具/文档示例/良性引用 → 忽略档：仅写全量 JSONL 留痕（每唯一内容一条）与 `adjudicated_low` 计数器 + ring 可见，**不进 SQLite 查询库、不进 KPI 审计面**（重启后 UI 里的 low 仅剩 ring 存活期）。**error/skipped**（调用失败/超时/队列满/单请求上限 4 个 job 之外的溢出/跨规则同内容去重吞掉的名字）→ **fail-open** 回经典立即记录（进 SQLite），绝不因判定器不可用而静音 guard。缓存：verdict 按 sha256(kind+rule+model+hit) LRU 缓存并持久化（`guard_verdicts.json`，只存 hash→verdict，无凭据内容）——历史回显只计费一次；error 不缓存；缓存重放不再触发 sink（每唯一内容一条记录），high 例外：每次出现都刷新拦截表。`secrets=block` 在判定通道开启时对 pattern 命中同样延后（由 LLM verdict 决定拦截，配置不再同步 400）；通道关闭时保留同步拦截。锁定测试：`internal/adjudicate/adjudicate_test.go`（high/medium/low/error、缓存、inflight、队列满、scrub、持久化）、`internal/forward/guard_adjudication_test.go`、`internal/app/guard_adjudication_integration_test.go`、`internal/cli/guard/guard_cli_test.go`、`internal/web/handlers_read_test.go`。

37. **精确匹配拦截（known-secret 家族）＝记录 + 拦截请求 + 拉黑会话，无条件执行；携带凭据溯源**：`known_secret`/`known_secret_encoded`（代理自有凭据的原文/编码形态精确匹配，零误报构造）与 `known_secret_fragmented`（跨请求拼装完成）不再按 `guard.secrets` 配置走 log/redact——一律：审计记录（`action=block`，无 verdict——经典记录）+ 400 终态拒绝 + 会话入拉黑表（与 high verdict 共表，仅显式解除；`无头不拉黑`：无 `x-claude-code-session-id` 的请求只拦截不拉黑）。拦截决定放在双扫描（secret+path）之后：同一请求的 strong path 命中仍拿到自己的计数/事件/审计记录，只有响应动作由拦截优先。`secrets=off` 仍完全关停 secret 通道（含精确拦截）。**溯源显示（operator 要求）**：拦截记录与拉黑 Reason 携带命中凭据的来源标签（`pool:<名>#<账号>/<字段>` / `oauth:<名>` / `memory:<名>`——身份字符串，由 providerbuild 采集、`guard.KnownIdentity` 在拦截时定位）与**掩码后的 key 片段**（头4…尾2，<8 字节全掩，maskSecretBytes 同款）——这是决策 17 红线的定向放宽：掩码片段不可还原凭据，且完整请求体本就在 admin-only 的 request log 里。**拦截请求落 request log**：终态 400 的 guard 拒绝现在写一条 request-log 记录（状态 400 + 原始请求体 + 拒绝消息），否则审计行的 request_id 指向空——Security 页的"查看原始请求"下钻以此为证据。锁定测试：`internal/forward/forward_test.go`（`TestServeGuardExactMatchIntercepts`——含溯源 Detail 断言与 request-log 断言、`TestServeGuardFragmentedSecretBlock`）、`internal/guard/scanner_test.go`（`TestKnownIdentity`）、`internal/app/guard_runtime_integration_test.go`。

38. **审计双层存储：SQLite 可查询库 + JSONL 全量留痕**：`guard.audit` 的持久化分两层——`security.db`（SQLite，WAL+busy_timeout，daemon writer 与 CLI 读者共存）是**唯一查询面**（/api/security、CLI audit、explain 的判定回读都查它），收录精确匹配、drift、medium/high、error/skipped；全量 JSONL（security-*.log，沿 logfile sink 轮转/retention）逐条收录**一切**记录（含 low）但不再被读取，仅供 tail。低风险（low）判定因此"重启即无查询痕迹"——UI 的 low tile 计数来自内存 ring；这是决策 36 忽略档的直接后果，用户已确认。旧 JSONL 历史不导入新库（文件原地保留）。锁定测试：`internal/observe/seclog/seclog_test.go`（双写、low 排除、newest-first/limit/Truncated、离线共存、权限）。
...
39. **high 判定内容拦截缓存（repeat interception）**：secret 类命中一旦被判定 high（fresh 或缓存重放），其命中字节的 sha256 进入持久化索引 `guard_blocked.json`（**只存 hash 与判定归属，绝不存命中字节**）。此后任何请求再携带相同字节：同步拦截——400 终态（request log 留痕）+ 审计记录（action=block、verdict=high、携带原判定的 reason/evidence/model）+ 会话入拉黑表（与精确匹配/高分判定共表，显式解除），**不再发起第二次判定调用**——机制对齐 known-secret 精确匹配通道（内容级、零误报前提是原判定本身）。path 类命中**不进**索引：短路径字面量跨上下文合法复现，路径判定保持逐次。会话拉黑表的持久化修复 restart drain 竞态：SIGINT 进程先关 listener 后 drain，晚写的高分 block 会被已启动的新进程下次写覆盖——blockStore 现按"上次已知盘面"合并收养晚写条目，自身 Unblock 的移除不被复活。锁定测试：`internal/adjudicate/adjudicate_test.go`（索引语义/drain 合并）、`internal/app/guard_adjudication_integration_test.go`（RepeatHighContentIntercepted）、`internal/admin/read_test.go`（counts 聚合）。
40. **unblock 留痕 + 请求行 guard 关联（查询时 join）**：两个 operator 可观测性契约。① 会话解除（`DELETE /api/security/blocks/<id>` / `guard unblock` / WebUI）不再是无痕动作：成功解除时 app 层写一条 `kind=unblock` 审计记录（Names=原 rule、RequestID=原触发请求、Detail 携带原拉黑的 kind/request/model 归属）+ 一条 guard live event；unblock **不 join 请求行标注**（它是会话级操作，不描述所指向的请求——挂在 200 请求上读作"该请求被解除"是歧义；其可见面是 Security feed 与拉黑卡）；拉黑表条目本身保持**删除语义**（它是"当前拦谁"的执法状态表，不是历史表——历史由审计记录承载，且 restart-drain 收养围栏依赖"缺席=主动解除"的集合差，见决策 39；**unblock 即终审**：拉黑条目记录判定背后的内容哈希与 verdict 缓存键，unblock 级联把 repeat 索引条目移入操作员豁免表 `guard_allowed.json`（同样字节不再拦截、不再送判，判定模型永不翻案）并删除对应缓存键；豁免可审计可撤销（`guard allowed`/`guard disallow`、`GET/DELETE /api/security/allowed`），撤销后内容回归逐次新判）。② 请求记录与判定/拦截记录的关联是**查询时 join**而非落盘字段：seclog 记录本就带 request_id（SQLite 有 `idx_audit_request`），admin 的装饰查询端口（`guardJoinQueries`）在 `/api/requests` summary 与 `/api/requests/<id>` 的 `guard` 字段按 request_id 批量 join——同步拦截（exact/fragmented/classic block、repeat）、异步判定 verdict（内存 ring 为 fresh 源含 low/cached，SQLite 库为持久源；重复条目合并为 ring 条目）与 unblock 留痕；request log 的 JSONL/索引**零改动**（guard 状态不进请求日志格式），无审计源时 fail-soft 缺省。同一请求命中多条规则的归并发生在**展示层**（审计库仍逐规则留痕——explain 按 name 定位、repeat 拦截按内容索引、KPI 按判定计数都依赖逐条记录）：归并单位是**请求 × channel（kind）**——Security feed 一行（`mergeSecurityFeed`：worst verdict 头条，per-rule verdict/理由为行内 segments〔`securitySegmentsHTML`〕）、详情 guard 组一行（`guardMarksDetailHTML` 同语义：同 verdict+理由共享一行理由，异构逐段）；请求行徽标按 verdict 计数合并（`judge·medium ×2`）。UI 三表单一份渲染（`requestRowHTML` + `guardMarksHTML` 徽标），详情展开按三区分层——labeled meta strip（`requestMetaHTML`：when/call/result/route/size）→ guard 轨迹（`guardMarksDetailHTML`：meta strip 同款扁平标签组，verdict 徽章引导行 + reason 单行省略号、全文进 tooltip）→ 对话/原始 body；unblock 行进 Security feed（kind 徽章 ok 色）但不进 Rule hits 计数。锁定测试：`internal/admin/read_test.go`（GuardJoin/WithoutGuardSources）、`internal/observe/seclog/store_test.go`（QueryByRequestIDs）、`internal/app/guard_adjudication_integration_test.go`（unblock 审计+事件）、`internal/web/handlers_read_test.go`（kind=unblock/detail guard）、`internal/web/jstests/pure.test.mjs`（徽标/详情/leaderboard 排除/feed 透传）。

41. **adjudicate.Service.mu → Proxy.mu 嵌套锁序（有意承认的第二条跨域边）**：`Service.Enqueue`/`process` 持 `adjudicate.Service.mu` 时调 `RuntimeConfig.AdjudicationConfig()`，其实现（`internal/app/guard_adjudication.go`）取 `Proxy.mu.RLock` 读当前代 config——形成 service.mu → Proxy.mu 嵌套，红线 2 的文字（跨域锁序只有 Proxy.mu → runtime.Manager）原本未承认它。接受理由：① 方向单向，Proxy.mu 的任何持锁路径都不回调 adjudicate.Service，与既有 Proxy.mu → runtime.Manager 边不成环；② 持锁期间无外部 I/O（RLock 内是纯内存 config 快照读）；③ 缩小临界区（锁外读 config）会重新打开 reload 换代与入队判定的竞态窗口。收紧前必须证明新锁序仍单向且无环。
42. **adjudicationSink 在 verdict 时刻取当前代 seclog，而非命中时刻快照**：`adjudicationSink.emit`/`Medium`/`Low` 在 sink 回调里 `SnapshotRuntime()` 取**当前代** `SecLog` 写审计记录，与红线 1「每个请求只捕获一次 runtimeSnapshot、异步分支只用该快照」的字面不同。有意为之：判定是请求外的异步通道，verdict 产生时原请求可能已结束、其快照所属代可能已退役——写当前代保证记录落在活着的查询面上；若改持命中代快照，迟到 verdict 会写进已 drain 的旧 logger 而被静默丢弃（条目 19 同款换代窗口）。代价：reload 换代后到达的 verdict 记录落在新一代的审计目录/文件，归属代与命中代可能不同——可观察语义不变（同一查询面），仅为落盘位置。

43. **同协议透传流截断时合成协议原生错误终态（不伪造成功终态）**：跨协议转换路径早已对"上游无终止信号 EOF"fail-closed；同协议 passthrough 此前是裸字节转发，上游中途弃流（实测 zhipu anthropic 端点在 thinking 阶段断连）时客户端只看到裸 EOF——严格客户端（Anthropic SDK/pi）硬错 `stream ended before message_stop`，宽容客户端（多数 OpenAI SDK）静默收下半截输出当完整答案。现状：`internal/protocol/stream_terminal_watch.go` 的 `TerminalWatcher` 原样转发字节、影子扫描终止信号，流已开始却在终止信号前结束（EOF 或读错）时向客户端追加协议原生错误终态（anthropic `event: error` / chat error chunk 不带 `[DONE]` / responses `response.failed`）。**有意不做的**：绝不合成 `message_stop`/`finish_reason:"stop"`/`[DONE]`/`response.completed` 这类成功终态——对 agent 客户端那是静默数据丢失（半截回复被当正常完成，不触发重试）；合成错误文案命中 agent 客户端（pi）的可重试错误文案正则（`stream ended before message_stop`、`ended without`、`stream ended before a terminal response event`），改文案必须保这三类子串。例外同样有意：上游自带错误终态不重复合成；anthropic 裸 `message_stop`（kimi 方言，无 stop_reason）对客户端可接受、不合成——缓存可回放性仍由 `StreamTerminalComplete` 单独把关（两问分离：客户端是否拿到终态 vs 是否可缓存回放）；空 body 不合成，保留零字节 200 失败锁定路径。watcher 在 effects 链之下，合成字节进 request log / cache recorder，天然不过缓存门禁。锁定测试：`internal/protocol/stream_terminal_watch_test.go`、`internal/app/stream_terminal_test.go`（复刻 zhipu 事故形态）。
