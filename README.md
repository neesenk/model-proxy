...
model-proxy login mimo             # 小米 MiMo API key（platform.xiaomimimo.com；可重复 -> 多账号）
model-proxy login typesafe         # TypeSafe API key（console.typesafe.ai；可重复 -> 多账号）
model-proxy login openrouter       # OpenRouter API key（openrouter.ai/settings/keys；可重复 -> 多账号）
model-proxy login opencode-go      # OpenCode Go 套餐 key（opencode.ai/auth 订阅 Go 后复制；可重复 -> 多账号）
...
- **Eval** — 评测观测面：Shadow Report 卡（`/api/shadow-report` 24h 窗口：每 route 的 primary vs shadow 状态一致率分档徽章、样本数、双侧延迟与 Δ）与 Fusion 卡（`/api/fusion`：每 workflow 的 runs/quorum/amplification/degraded 汇总 + 最近 runs 逐条 quorum/drafts/legs/judge/synth 状态，leg 详情进悬停 tooltip）。Status→Schedule 卡每个 route 另有 **Test** 按钮（`test <model>` 的行内版：逐 target 真实上游探测，✓/✗ + HTTP 状态 + 延迟就地展示）；Requests 详情展开底部有 **Replay** 条（`replay <id> --to <provider>` 的行内版：一次性 force-provider 重答，响应体折叠懒渲染）；Status→Models 区有 **Refresh Catalog**（`models pull` 的 Web 版，强刷 models.dev 缓存）。
...
- **Requests** — 请求日志查询（需 `request_log.enabled`）：按 session/agent/model/provider/errors 过滤（Model/MCP 两个流，MCP 流的表格换 Server/Tool/Account 列语义），表格与 Live 页**同列同渲染器**（time/agent/session/status/model/provider/ms/tokens——含 cache read；bytes 列已移除），点击行展开请求详情：**默认渲染人读对话视图**（角色分栏的消息流——user 提示词、assistant 回复、thinking 与工具结果折叠、工具调用与 JSON 工具结果渲染为可读的 `key: value` 文本而非 JSON 语法、token 用量行；请求侧最近 4 轮直读，更早历史折叠零内容、展开即**平铺完整对话**进滚动框按 25 轮分块加载——千轮会话展开 ~35KB 首块秒开，滚动即续），**原始 request/response body 折叠在下方，首次展开时才渲染且 64KB 分块滚动加载**（未点开零开销，多 MB body 不再卡死页面）；影子评测的记录带 `shadow` 徽标。顶部 **session 下拉**（选项来自 `/api/sessions`）选中后，表格上方渲染与 Live 页**同一实现**的会话视图：汇总 chips（请求数 / input+output / 缓存读写 / 平均延迟 / 错误 / 等价成本 / model、provider）+ 健康度 chips（跨度/活跃时长、p50/p95 延迟、ttft p50、failover 次数、缓存命中率、tok/s、模型分布、shadow 数）+ **Trace 时间线**（条形悬停显示请求摘要与「用户输入 + 模型返回」两段摘录，条形点击**定位到该请求行**——滚动到视野中央、闪烁标记并展开 inline 详情；长会话的表格滚到几百行深时会话视图**悬浮固定**在顶栏下方不跟丢，表头联动钉在悬浮视图正下方）；表格按 `session=` 过滤，仍可与 model/provider/errors 叠加。**过滤器状态随 URL hash 保持**（`#requests/model_all?session=…&agent=…`，MCP 流为 `#requests/mcp_all`，只携带非默认值）：刷新页面或分享链接落在同一视图而不是回到全量列表，浏览器前进/后退在过滤器状态间切换。
- **Security** — 安全视图（审计记录需 `guard.audit`；30s 自动刷新，交互中自动让位）：顶部 KPI 行（拉黑会话 / verdict 汇总——**三级判定**：high 拦截拉黑、medium 只记录、low 忽略档（仅内存 ring 存活期可见，不进查询库）；判定通道 LLM 用量——真实模型调用次数与 in/out token，缓存命中与在途去重不计费，通道未配置时显示单个 off tile）；**Blocked sessions** 卡列出被拉黑的会话——高 verdict **与精确匹配拦截**（代理自有凭据 known_secret* 家族命中：记录 + 拒绝请求 + 拉黑会话，零误报通道无条件执行）共用同一张表，逐行解除或一键全部解除（持久化跨重启），session 列点击跳转 Requests 会话视图（hash 下钻，Back 返回）；**Rule hits** 卡按规则聚合**全量审计窗口**命中数（所有 kind、整个 30d 保留期，独立加载、不受 Activity 的 kind/时间窗口过滤器影响；审计记录 + 被屏蔽的 low），高频 pattern 规则在判定通道关闭时标 "noisy — adjudicate can suppress" 降噪提示，点击规则行下钻 Activity 到该规则（再点/✕ chip 清除）；**Activity** 合并时间线把审计记录（秘密/路径类型、takeover 漂移）与 AI 判定 verdict 放进同一 feed 新到旧展示——fresh verdict 双写去重（同一事件只显一行，judge 归属随行），被屏蔽的 low verdict 只在这里可见；每行（含 cached 标记的 ai· verdict 行）可 **analyze** 行内展开：按需重扫请求体定位每处命中——LLM 判定块（verdict/判定逻辑/事实依据/模型/cached）在前，命中按规则分组、多处命中编号展示、命中片段在上下文中高亮折行，展开状态在 Refresh/自动刷新后保留（结果页内缓存，不重复请求）；过滤器：kind 与时间窗口（All Time/24h/7d/30d）服务端生效 + Show More 逐级加深（至 1,000 行）、verdict（含 Medium）/规则客户端过滤，全部进 URL hash（刷新/分享保留）。**存储双层**：SQLite 查询库（`security.db`——精确匹配、drift、medium/high、error/skipped 落库并关联原始请求）+ JSONL 全量留痕（含 low，仅供 tail）；旧 JSONL 历史不迁移。精确匹配行直接显示命中凭据的来源（哪个池/账号/字段的 key）与掩码 key 片段；verdict 行直接显示 LLM 的判定逻辑与事实依据原文。带请求的行可点 req 链接下钻原始请求：Requests 页顶部固定展示该请求 + guard 命中高亮定位（explain 重扫）；被拦截的 400 请求同样落 request log 可下钻。全部只展示类型名与路由元数据，匹配内容永不进入查询面（analyze 的片段来自 admin 本可读的 request log，按需重扫、不落盘）。每个客户端会话的 token 等价成本汇总在 `/api/sessions`（Requests 页同源数据）。
- **Live**（Requests 页左侧边栏——`Model`/`MCP` 分组下的 `Live` 项，hash `#requests/<stream>_live`，即 `model_live` / `mcp_live`）— 实时请求监视（SSE 推送）：哪个 agent 正在发请求、路由到哪个上游、状态/token/耗时——抓「疯狂重试的 agent」就靠它；MCP 档监视网关交换（initialize/tools 握手与调用，agent 来自 clientInfo 归属）。顶部 **session 选择器**（Model 档）可选一个客户端会话（选择随 URL hash 保持，`#requests/model_live?session=…`——刷新/分享链接/前进后退还原同一会话视图；旧 `#status/live`、`#requests/live` 链接自动重写），看该会话的请求分析：**Trace 时间线**（请求 span 泳道图 + 累计 token 曲线，琥珀点＝发生过 failover 重试，红条＝错误响应，条形悬停显示该请求摘要（时间/agent/model→provider/状态/延迟/token）并懒加载**两段内容摘录**（这轮用户输入 + 模型返回，各 ≤220 字符、无标签前缀、以弱/正常文本区分，与详情行共享缓存；纯工具调用轮显示 `[tool_use: …]` 标记、纯思考轮 `[thinking]`、错误响应显示 error 消息——coding agent 的大量工具轮不再空白），条形可点开详情；**长空闲间隙自动压缩**——2 分钟无请求即断轴并以 ⫽ 分隔，短突发在跨天会话里仍占可读宽度；**横向拖拽缩放**进任意时间窗，zoom 状态跨 SSE 重渲染存活，reset 一键还原；泳道超过 14 条时压缩行高；成本保持会话级汇总——定价归服务端）+ 汇总 chips（请求数 / input+output / 缓存读写 / 平均延迟 / 错误 / 等价成本 / model、provider），实时行与持久化行合并（表格与 Requests 页同列同渲染器、表头同样悬浮跟随），逐行可展开 body（详情与 Requests 页同一人读对话视图，原始 body 折叠），会话视图悬浮固定在顶栏下方、表头联动跟随。会话 id 来自 `request_log.session_headers` 允许列表（Claude Code/OpenCode 默认带；pi 需 takeover 模板写入的 `compat.sendSessionAffinityHeaders`）；无会话头的客户端（Codex）回落请求体 `client_metadata.session_id`（Responses 协议惯例，仅观测与 guard 会话维度，不改路由粘性）。
...
JSON 接口在 `/api/*`（`status` / `models` / `logs` / `config` / `accounts`（含 `…/<id>/test`）/ `tokens` / `stats` / `agents` / `analytics` / `sessions` / `requests` / `security` / `shadow-report` / `fusion` / `mcp`（含 `…/test`、`…/analytics`）/ `events` / `pin` / `quota/refresh` / `login/*`）；底层契约（请求/响应 shape、stats 口径）见仓库根目录 `docs/web-api.md`。前端是嵌入式的静态资源（`internal/web/assets/`，`go:embed`），无独立构建步骤。
...
转换是 per-target 的；同协议目标通常保持字节级透传，Codex Responses 例外：会清理其明确拒绝的采样/输出上限参数。Responses 客户端跨协议使用 `previous_response_id` 时，proxy 以 30 分钟短期本地状态展开完整历史（有界、0600 原子持久化；只缓存 completed/token-limit incomplete）。客户端 `stream` 与上游实际模式相反时会聚合 SSE 或合成 SSE。跨协议 4xx 保留 HTTP 状态和错误信息并改写为客户端错误格式；已知无法无损表达的请求先尝试兼容 target，最终按客户端格式返回 400 `unsupported_protocol_conversion`；缺协议终止事件、scanner 失败或工具 arguments JSON 未闭合时 fail-closed。同协议透传流在上游无终止信号就断流时同样 fail-closed：字节原样转发，末尾追加协议原生错误终态（anthropic `error` 事件 / chat error chunk / responses `response.failed`）——绝不伪造成功终态，宽容客户端不再静默收下半截输出，严格客户端拿到可重试的协议级错误。document/input_file/file、tool_result 图片与 `is_error`、hosted web_search/tool_search（含发现工具物化）、citations 和 signed/redacted reasoning replay 均可转换或采用明确可见降级；到 Anthropic 的转换自动生成 prompt-cache breakpoints，Chat↔Responses 保留 cache key/retention。内联图片跨协议时限制为 4 MiB/4096px，413 时仅压缩重试一次（1 MiB/2048px）。MCP namespace 经 Chat/Anthropic 目标均可双向压平与恢复；custom/freeform 工具经 Chat 双向保留，经 Anthropic 时因没有等价 raw-input 契约而明确拒绝。reasoning effort 按 provider 方言渲染。unsupported server tools、audio、多 choice/logprobs 等不可表达特性会被能力扫描器拒绝，不再静默丢弃。
...
影子请求不污染生产（不进熔断/统计/粘性）；Web UI Requests 页里影子记录带 `shadow` 徽标，点任意一条可用 `replay` 继续深挖。评测维度目前是客观指标（状态/延迟/大小），LLM judge 胜率未做。
...
```bash
model-proxy serve
python3 examples/demo.py --port 15722 "hello" glm-5.3
python3 examples/demo.py --port 15722 --protocol codex "hello" gpt-5.6-luna
```
