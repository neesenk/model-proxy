// Package session owns the split-exfiltration session windows: a bounded LRU
// mapping x-claude-code-session-id → the tail of that session's recent
// request bodies plus cross-request fragment progress, used by the guard's
// known_secret_fragmented detection. The windows may CONTAIN CREDENTIALS:
// they live in process memory only and are never logged, serialized,
// persisted, or exposed via any API/event. The store is process-lifetime,
// cross-generation runtime observation state (like the metrics store),
// created once at construction and untouched by reload — it is deliberately
// NOT part of RuntimeSnapshot.
package session
...
// Store is a bounded LRU mapping x-claude-code-session-id → the tail of that
// session's recent request bodies + fragment progress, for split-exfiltration
// detection (a known credential smuggled out in pieces, one fragment per
// request). The session key is CLIENT-CONTROLLED: an agent that omits or
// rotates the header never engages the channel, and 256 distinct ids flush
// the whole LRU — a bounded heuristic against cooperative clients, not a
// detection guarantee against deliberate evasion (decision 20).
//
...
func NewStore() *Store {
	return &Store{ll: list.New(), items: map[string]*list.Element{}}
}
...
func (s *Store) Snapshot(id string, sc *guard.Scanner) (tail []byte, progress []int, known, knownOK bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
	el, ok := s.items[id]
	if !ok {
...
func (s *Store) Add(id string, body []byte, sc *guard.Scanner, progress []int, reset []bool, bodyKnown bool) {
	if len(body) == 0 {
		return
	}
