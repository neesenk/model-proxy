...
func TestArchitectureRootInteractionContracts(t *testing.T) {
	...
	t.Run("application owns command registration and main only crosses the OS boundary", func(t *testing.T) {
		...
		if got := namedCallCountInNode(constructor.Body, "ProcessCommand"); got != 19 {
			t.Errorf("newApplication processCLICommand registrations = %d, want 19 concrete command bindings", got)
		}
