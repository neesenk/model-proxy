...
func DoReplay(base, id, provider string) ([]byte, error) {
	...
	if status != 200 {
		return nil, fmt.Errorf("daemon returned HTTP %d: %s", status, display.Truncate(string(recBody), 200))
	}
	var got struct {
	...
	fmt.Fprintf(os.Stderr, "%s replay %s → %s (HTTP %d)\n", display.Dim("•"), id, provider, resp.StatusCode)
	body, _ := io.ReadAll(resp.Body)
	if resp.StatusCode >= 400 {
		return nil, fmt.Errorf("%s", display.Truncate(strings.TrimSpace(string(body)), 400))
	}
	return body, nil
}
