...
func TestCLI_RoutesList(t *testing.T) {
	cfgPath := writeTempConfig(t, routesCmdConfig)
	stdout, _, code := runCLI(t, "routes", cfgPath)
	if code != 0 {
...
func TestCLI_RoutesDetail(t *testing.T) {
	cfgPath := writeTempConfig(t, routesCmdConfig)
	stdout, _, code := runCLI(t, "routes", cfgPath, "kimi-k3")
	if code != 0 {
	...
	stdout, _, _ = runCLI(t, "routes", cfgPath, "deepseek-v4-pro")
...
func TestCLI_RoutesUnknownModel(t *testing.T) {
	cfgPath := writeTempConfig(t, routesCmdConfig)
	_, stderr, code := runCLI(t, "routes", cfgPath, "nope")
	if code != 1 {
