...
func TestCLI_LoginFromEnv(t *testing.T) {
	...
	defer srv.Close()
	cfg := writeTempConfig(t, loginImportConfig(srv.URL))
	home := t.TempDir()
	t.Setenv("MP_CLI_FROM_ENV_KEY", "env-key-abc")

	stdout, stderr, code := runCLIWithHome(t, home, "login", cfg, "zhipu", "--from-env", "MP_CLI_FROM_ENV_KEY", "--label", "env")
	if code != 0 {
...
func TestCLI_LoginFromEnv_MissingVar(t *testing.T) {
	cfg := writeTempConfig(t, loginImportConfig(""))
	t.Setenv("MP_CLI_MISSING_KEY", "")
	_, stderr, code := runCLI(t, "login", cfg, "zhipu", "--from-env", "MP_CLI_MISSING_KEY")
	if code == 0 {
...
func TestCLI_LoginFromCodex(t *testing.T) {
	...
	stdout, stderr, code := runCLIWithHome(t, home, "login", cfg, "codex", "--from-codex")
	if code != 0 {
...
func TestCLI_LoginFromCodex_MissingFile(t *testing.T) {
	cfg := writeTempConfig(t, loginImportConfig(""))
	_, stderr, code := runCLI(t, "login", cfg, "codex", "--from-codex")
	if code == 0 {
...
func TestCLI_LoginFromCodex_WrongProvider(t *testing.T) {
	cfg := writeTempConfig(t, loginImportConfig(""))
	_, stderr, code := runCLI(t, "login", cfg, "zhipu", "--from-codex")
	if code == 0 {
