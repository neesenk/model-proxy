// Package audit owns the `audit` command: the offline viewer for the
// security audit log written by the guard pipeline and `doctor --live`.
package audit
...
func RenderAudit(dir string, opts AuditOpts, now time.Time) (string, error) {
	...
	if opts.Stats {
		...
		if result.Skipped > 0 {
			out += fmt.Sprintf("  (%d unreadable %s skipped)\n", result.Skipped, cliframework.Plural(result.Skipped, "line", "lines"))
		}
		return out, nil
	}
	if opts.JSON {
...
func FormatAuditStats(stats *AuditStats, dir string) string {
	...
	out := fmt.Sprintf("security audit stats  range: %s .. %s  total: %d %s\n",
		window(stats.From), window(stats.To), stats.Total, cliframework.Plural(stats.Total, "record", "records"))
	if stats.Total == 0 {
