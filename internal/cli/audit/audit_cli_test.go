...
func TestAuditCLISubprocess(t *testing.T) {
	...
	stdout, stderr, code := clitest.RunCLIWithHome(t, home, "audit", cfgPath)
	if code != 0 {
	...
	stdout, stderr, code = clitest.RunCLIWithHome(t, home, "audit", cfgPath, "--json")
	if code != 0 {
	...
	stdout, stderr, code = clitest.RunCLIWithHome(t, home, "audit", cfgPath, "--stats")
	if code != 0 {
	...
	stdout, stderr, code = clitest.RunCLIWithHome(t, home, "audit", cfgPath, "--kind", "bogus")
	if code != 1 {
	...
	emptyHome := t.TempDir()
	stdout, _, code = clitest.RunCLIWithHome(t, emptyHome, "audit", cfgPath)
	if code != 0 {
...
func TestAuditCLIRespectsConfigAuditPath(t *testing.T) {
	...
	stdout, _, code := clitest.RunCLIWithHome(t, home, "audit", cfgPath)
	if code != 0 {
