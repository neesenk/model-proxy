...
func TestRenderStatsCLI(t *testing.T) {
	...
	resp := stats.StatsResp{From: minute - 60, To: minute, Bucket: 60, Buckets: []observestats.Bucket{
	...
	out, err := stats.RenderStats(listen, stats.StatsOpts{})
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := stats.RenderStats(listen, stats.StatsOpts{Bucket: "10m"}); err != nil {
		t.Fatal(err)
	}
	mu.Lock()
	query = gotQuery
	...
	outJSON, err := stats.RenderStats(listen, stats.StatsOpts{JSON: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestBucketLabel(t *testing.T) {
	...
	for _, c := range cases {
		if got := stats.BucketLabel(c.secs); got != c.want {
			t.Errorf("stats.BucketLabel(%d) = %q, want %q", c.secs, got, c.want)
		}
	}
}
...
func TestParseStatsFlags(t *testing.T) {
	cases := []struct {
		name string
		args []string
		want stats.StatsOpts
	}{
		{"empty", nil, stats.StatsOpts{}},
		{"space-separated", []string{"--from", "2026-01-01", "--to", "2026-02-01",
			"--provider", "zhipu", "--model", "glm-5.2", "--bucket", "5m", "--json"},
			stats.StatsOpts{From: "2026-01-01", To: "2026-02-01", Provider: "zhipu",
		...
		{"value at end without arg", []string{"--from"}, stats.StatsOpts{}},
		{"unknown flag ignored", []string{"--bogus", "x"}, stats.StatsOpts{}},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := stats.ParseStatsFlags(tc.args)
			if got != tc.want {
				t.Errorf("stats.ParseStatsFlags(%v) = %+v, want %+v", tc.args, got, tc.want)
			}
		})
	}
}
...
func TestFormatStatsSummary(t *testing.T) {
	...
	out := stats.FormatStatsSummary(stats.StatsSummary{From: 1700000000, To: 1700003600})
	...
	out = stats.FormatStatsSummary(stats.StatsSummary{
...
func TestSummarizeStats(t *testing.T) {
	s := stats.SummarizeStats(stats.StatsResp{From: 1, To: 2, Buckets: []observestats.Bucket{
...
func TestFormatStatsTable(t *testing.T) {
	...
	out := stats.FormatStatsTable(stats.StatsResp{From: 1700000000, To: 1700003600, Bucket: 60})
	...
	resp := stats.StatsResp{From: 1700000000, To: 1700003600, Bucket: 3600, Buckets: []observestats.Bucket{
		{Provider: "zhipu", Model: "glm-5.2", Minute: 1700000000, Requests: 100, Failovers: 2, Failures: 1, Input: 5000, Output: 3000},
	}}
	out = stats.FormatStatsTable(resp)
...
// into stats.StatsOpts (the new analytics-routing flags).
func TestStatsFlags_GranularityCost_Parsed(t *testing.T) {
	o := stats.ParseStatsFlags([]string{"--granularity", "month", "--cost", "--provider", "deepseek"})
...
func TestStatsFlags_GranularityCost_DefaultOff(t *testing.T) {
	o := stats.ParseStatsFlags([]string{"--from", "1", "--bucket", "1h"})
...
func TestStatsFlags_GranularityCost_EqualsForm(t *testing.T) {
	o := stats.ParseStatsFlags([]string{"--granularity=day", "--cost"})
...
func TestRenderStatsCLI_AnalyticsPath(t *testing.T) {
	...
	out, err := stats.RenderStats(listen, stats.StatsOpts{Granularity: "month", Cost: true})
	if err != nil {
		t.Fatal(err)
	}
	mu.Lock()
	...
	sawAnalytics = false
	mu.Unlock()
	if _, err := stats.RenderStats(listen, stats.StatsOpts{Cost: true}); err != nil {
		t.Fatal(err)
	}
	mu.Lock()
...
func TestRenderStatsCLI_AnalyticsJSON(t *testing.T) {
	...
	defer up.Close()
	listen := strings.TrimPrefix(up.URL, "http://")
	out, err := stats.RenderStats(listen, stats.StatsOpts{Granularity: "day", JSON: true})
	if err != nil {
		t.Fatal(err)
	}
	if out != body {
...
func TestFormatAnalyticsTable(t *testing.T) {
	cost := 0.12
	resp := stats.AnalyticsResp{Granularity: "month"}
	resp.Series = []struct {
	...
	out := stats.FormatAnalyticsTable(resp, false)
	...
	if !strings.Contains(out, cliframework.CompactNum(5)) { // 3+2 requests
		t.Errorf("month-1 requests sum missing:\n%s", out)
	}

	// With cost: 7-column table; priced series shows $0.24 (0.12+0.12),
	// unpriced shows n/a.
	out = stats.FormatAnalyticsTable(resp, true)
...
func TestRenderAgentsCLI(t *testing.T) {
	resp := stats.AgentResp{From: 1, To: 2, Bucket: 60, Buckets: []observestats.AgentBucket{
	...
	out, err := stats.RenderAgents(listen, stats.StatsOpts{ByAgent: true})
	if err != nil {
		t.Fatal(err)
	}
	...
	outJSON, err := stats.RenderAgents(listen, stats.StatsOpts{ByAgent: true, JSON: true})
	if err != nil {
		t.Fatal(err)
	}
	if outJSON != string(raw) {
...
func TestRenderAgents_ProviderModelFilter(t *testing.T) {
	...
	if _, err := stats.RenderAgents(listen, stats.StatsOpts{ByAgent: true, Provider: "zhipu", Model: "glm-5.2"}); err != nil {
		t.Fatal(err)
	}
}
