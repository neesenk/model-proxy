// Package status owns the daemon status read commands: `serve status`
// (providers/schedule/quota/tokens/logs), `schedule`, and `routes`.
package status
...
func compactNum(n uint64) string { return cliframework.CompactNum(n) }
...
func RenderProviders(st *StatusResp) string {
	...
	fmt.Fprintf(&b, "%s (%d)\n", display.Bold("Providers"), len(names))
	hdr := fmt.Sprintf("  %s  %s  %8s  %9s  %5s  %8s  %6s  %6s  %s",
		display.Pad("PROVIDER", 16), display.Pad("HEALTH", 13), "REQS", "FAILOVERS", "429", "FAILURES", "LAT", "TTFT", "LAST")
	fmt.Fprintln(&b, display.Dim(hdr))
	for _, name := range names {
		label, color := HealthLabel(st.Health[name])
		c := st.Counters[name]
		fmt.Fprintf(&b, "  %s  %s  %8s  %9s  %5s  %8s  %6s  %6s  %s\n",
			display.Pad(name, 16),
			color(display.Pad(label, 13)),
...
func HealthLabel(h StatusHealth) (string, func(string) string) {
	...
	case h.CircuitState == "open":
		return "circuit open", display.Red
	case h.CircuitState == "half_open":
		return "half-open", display.Red
	case h.RateLimitedUntil != "" && h.RateLimitKind == "quota":
		return "rl:quota", display.Yellow
	case h.RateLimitedUntil != "" && h.RateLimitKind == "daily":
		return "rl:daily", display.Yellow
	case h.RateLimitedUntil != "":
		return "rate-limited", display.Yellow
	case h.Available:
		return "available", display.Green
	default:
		return "unavailable", display.Dim
	}
}
...
func RenderTokens(t *TokensResp) string {
	...
	fmt.Fprintf(&b, "%s (%d %s · %s requests)\n",
		display.Bold("Tokens"), len(t.Usage), cliframework.Plural(len(t.Usage), "model", "models"), compactNum(totalReqs))
	...
	for _, name := range names {
		...
		for _, w := range q.Windows {
			...
			if !w.ResetsAt.IsZero() {
				resets = display.Dim("  resets " + display.FormatResetAt(w.ResetsAt.UnixMilli()))
			}
			fmt.Fprintf(&b, "      %s  %5s  %s%s\n",
				display.Pad(label, 22), pctStr, display.ProgressBar(usedPct, 16), resets)
		}
	}
	return b.String()
}
...
func RenderStatus(listen string, opts StatusOpts) (string, error) {
	...
	fmt.Fprintf(&b, "%s  %s · %s · %s\n\n",
		display.Bold("model-proxy"), display.Dim("v"+st.Version), display.Dim(st.Uptime), display.Dim(st.Listen))
