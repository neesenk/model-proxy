...
func TestDoUnfreeze(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if body.Provider == "empty" {
			clitest.WriteJSON(w, http.StatusOK, map[string]any{"cleared": []string{}, "model_locks_cleared": 0})
			return
		}
		clitest.WriteJSON(w, http.StatusOK, map[string]any{"cleared": []string{"zhipu"}, "model_locks_cleared": 2})
	}))
	defer srv.Close()
