...
// CmdDoctor implements `model-proxy doctor [--live]`. The root wrapper loads
// the config and supplies its path (takeover backups live next to it).
func CmdDoctor(args []string, cfg *configdomain.Config, cfgPath string) {
...
// peakSummary renders a configdomain.PeakConfig as "09:00-12:00(×2), 14:00-18:00(×2)" or "-".
func PeakSummary(ph configdomain.PeakConfig) string {
	if len(ph) == 0 {
		return "-"
	}
