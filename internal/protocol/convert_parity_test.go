...
func TestParity_ReasoningEffortDialects(t *testing.T) {
	...
	for _, pid := range []string{"zhipu", "volcengine", "kimi-code", "deepseek"} {
		out, err := convertResponsesRequestToOpenAIFor([]byte(mk("high")), convertReqOpts{ReasoningDialect: ReasoningThinking, ImageOK: true})
		if err != nil {
			t.Fatal(err)
		}
		...
		out2, _ := convertResponsesRequestToOpenAIFor([]byte(mk("none")), convertReqOpts{ReasoningDialect: ReasoningThinking, ImageOK: true})
	...
	out4, err := convertResponsesRequestToOpenAIFor([]byte(mk("low")), convertReqOpts{ReasoningDialect: ReasoningEffort, ImageOK: true})
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, tc := range []struct {
		...
		{"zhipu", ReasoningThinking},
		{"qwen-plan", ReasoningEnableThinking},
		{"aqp", ReasoningOpenRouter},
	} {
		out5, _ := convertResponsesRequestToOpenAIFor([]byte(noR), convertReqOpts{ReasoningDialect: tc.dialect, ImageOK: true})
		m := unmarshalMap(t, out5)
		if _, has := m["thinking"]; has {
			t.Errorf("%s: no reasoning → thinking must be absent", tc.pid)
		}
		if _, has := m["enable_thinking"]; has {
			t.Errorf("%s: no reasoning → enable_thinking must be absent", tc.pid)
		}
	}
}
